...
def test_photometry_stream_put_access(
    upload_data_token_no_groups,
    upload_data_token_no_groups_stream_two,
    public_source,
    public_stream,
    public_stream2,
    ztf_camera,
    ...
    # this token doesn't have sufficient stream access to see the original stream relationship
    status, data = api(
        'PUT',
        'photometry',
        data={
        ...
        token=upload_data_token_no_groups_stream_two,
    )
    assert status == 400
    assert data['status'] == 'error'
